...
| 클래스 | 책임 |
|--------|------|
| `LevelProgressManager` | `maxClearedLevel` 로컬/서버 동기화, Debug 오버라이드, `MissionData.isClear` 파생 적용 |
...
- 런타임 `isClear` 파생: `isClear = (levelIndex <= effectiveMaxCleared)`
  - 예: `effectiveMaxCleared=3` → 레벨 1~3 완료, 레벨 4까지 해금

## 테스트용 진행도 오버라이드

- Debug 설정은 `LevelProgressManager` 인스펙터의 `Debug/Test` 섹션(`_useDebugCurrentLevel`, `_debugCurrentLevel`)에 둔다.
- `ApplyToMissionTable(table)`은 항상 `GetEffectiveMaxClearedLevel()`을 사용하므로, `LevelMapManager` / `LevelUIButtonController` / `MissionManager` 호출이 같은 기준을 공유한다.
- `_debugCurrentLevel`은 현재 플레이 레벨(1-base)이며, 적용 시 `effectiveMaxCleared = currentLevel - 1`로 변환한다. (예: Current Level=4 → 1~3 클리어, 4가 현재 위치)
...
classDiagram
  class LevelProgressManager {
    ...
    -GetEffectiveMaxClearedLevel() int
